...
var _PaginatedMessage = class _PaginatedMessage {
  ...
  addAsyncPageEmbed(embed) {
    return this.addPage(async () => ({ embeds: utilities.isFunction(embed) ? [await embed(new discord_js.EmbedBuilder())] : [embed] }));
  }
  /**
  ...
  addPageEmbeds(embeds) {
    let processedEmbeds = utilities.isFunction(embeds) ? embeds(
  ...
  addAsyncPageEmbeds(embeds) {
    return this.addPage(async () => {
      let processedEmbeds = utilities.isFunction(embeds) ? await embeds(
        new discord_js.EmbedBuilder(),
        new discord_js.EmbedBuilder(),
        new discord_js.EmbedBuilder(),
        new discord_js.EmbedBuilder(),
  ...
  addPageAction(action, index) {
    if (index < 0 || index > this.pages.length - 1) throw new Error("Provided index is out of bounds");
    const pageActionAtIndex = this.pageActions[index] ?? /* @__PURE__ */ new Map();
    if (actionIsLinkButton(action)) {
  ...
  async run(messageOrInteraction, target) {
    if (!messageOrInteraction.channel) {
      ...
      if (shouldEmitWarning && isInteraction && messageOrInteraction.client.options.partials?.includes(discord_js.Partials.Channel)) {
        shouldEmitWarning = false;
      }
      if (shouldEmitWarning && !isInteraction && messageOrInteraction.client.options.partials?.includes(discord_js.Partials.Channel) && new discord_js.IntentsBitField(messageOrInteraction.client.options.intents).has(discord_js.GatewayIntentBits.DirectMessages)) {
        shouldEmitWarning = false;
      }
  ...
  async resolvePage(messageOrInteraction, target, index) {
    ...
    if (!utilities.isNullish(message)) {
      return message;
    }
  ...
  async getPageOptions(index) {
    ...
    return utilities.isFunction(page) ? page(index, this.pages, this) : page;
  }
  /**
   * Sets up the message.
   *
  ...
  setUpCollector(messageOrInteraction, targetUser) {
    if (this.pages.length > 1) {
      this.collector = new discord_js.InteractionCollector(targetUser.client, {
        filter: /* @__PURE__ */ __name((interaction) => {
          if (!utilities.isNullish(this.response) && interaction.isMessageComponent()) {
            const customIdValidation = this.actions.has(interaction.customId) || this.pageActions.some((actions) => actions?.has(interaction.customId));
  ...
  async handlePageLoad(page, index) {
    ...
    const clonedTemplate = utilities.deepClone(this.template);
    ...
    return Promise.all(
      actions.map(async (interaction) => {
        if (isMessageButtonInteractionData(interaction)) {
          return new discord_js.ButtonBuilder(interaction);
        }
        if (isMessageUserSelectInteractionData(interaction)) {
          return new discord_js.UserSelectMenuBuilder(interaction);
        }
        if (isMessageRoleSelectInteractionData(interaction)) {
          return new discord_js.RoleSelectMenuBuilder(interaction);
        }
        if (isMessageMentionableSelectInteractionData(interaction)) {
          return new discord_js.MentionableSelectMenuBuilder(interaction);
        }
        if (isMessageChannelSelectInteractionData(interaction)) {
          return new discord_js.ChannelSelectMenuBuilder(interaction);
        }
        if (isMessageStringSelectInteractionData(interaction)) {
          return new discord_js.StringSelectMenuBuilder({
            ...interaction,
  ...
  async handleCollect(targetUser, channel, interaction) {
    if (interaction.user.id === targetUser.id) {
      ...
      await interaction.reply(
        utilities.isObject(interactionReplyOptions) ? interactionReplyOptions : { content: interactionReplyOptions, ephemeral: true, allowedMentions: { users: [], roles: [] } }
      );
    }
  }
  /**
  ...
  applyFooter(message, index) {
    ...
    const embedsWithFooterApplied = utilities.deepClone(message.embeds);
    ...
    if (embedsWithFooterApplied.length > 0) {
      ...
      const jsonTemplateEmbed = discord_js.isJSONEncodable(templateEmbed) ? templateEmbed.toJSON() : templateEmbed;
      if (discord_js.isJSONEncodable(lastEmbed)) {
  ...
  mergeEmbeds(templateEmbed, pageEmbeds) {
    const mergedEmbeds = [];
    const jsonTemplate = discord_js.isJSONEncodable(templateEmbed) ? templateEmbed.toJSON() : templateEmbed;
  ...
  mergeArrays(template, array) {
    if (utilities.isNullish(array)) {
      return template;
    }
    if (utilities.isNullish(template)) {
      return array;
    }
...
__publicField(_PaginatedMessage, "defaultActions", [
  {
    ...
    type: discord_js.ComponentType.StringSelect,
    options: [],
    run: /* @__PURE__ */ __name(({ handler, interaction }) => interaction.isStringSelectMenu() && (handler.index = parseInt(interaction.values[0], 10)), "run")
  },
  {
    customId: "@sapphire/paginated-messages.firstPage",
    style: discord_js.ButtonStyle.Primary,
    emoji: "\u23EA",
    type: discord_js.ComponentType.Button,
    run: /* @__PURE__ */ __name(({ handler }) => handler.index = 0, "run")
  },
  {
  ...
  },
  {
    ...
    emoji: "\u23E9",
    type: discord_js.ComponentType.Button,
    run: /* @__PURE__ */ __name(({ handler }) => handler.index = handler.pages.length - 1, "run")
  },
  {
...
var _LazyPaginatedMessage = class _LazyPaginatedMessage extends PaginatedMessage {
  ...
  addPageEmbed(embed) {
    return this.addPage(() => ({ embeds: typeof embed === "function" ? [embed(new discord_js.EmbedBuilder())] : [embed] }));
  }
  addPageEmbeds(embeds) {
    return this.addPage(() => {
      let processedEmbeds = utilities.isFunction(embeds) ? embeds(
...
var _PaginatedFieldMessageEmbed = class _PaginatedFieldMessageEmbed extends PaginatedMessage {
  constructor() {
     ...
     */
    __publicField(this, "embedTemplate", new discord_js.EmbedBuilder().toJSON());
    /**
  ...
  generatePages() {
    ...
    for (let i = 0; i < this.totalPages; i++) {
      const clonedTemplate = new discord_js.EmbedBuilder(template);
      const fieldsClone = utilities.isNullishOrEmpty(template.fields) ? [] : [...template.fields];
      if (fieldsClone.length > 0) clonedTemplate.setFields();
      if (utilities.isNullish(template.color)) clonedTemplate.setColor("Random");
  ...
  resolveTemplate(template) {
    if (utilities.isFunction(template)) template = template(new discord_js.EmbedBuilder());
    return (discord_js.isJSONEncodable(template) ? template : new discord_js.EmbedBuilder(template)).toJSON();
  }
};
...
var _PaginatedMessageEmbedFields = class _PaginatedMessageEmbedFields extends PaginatedMessage {
  ...
  make() {
    if (!this.items.length) throw new Error("The items array is empty.");
    if (this.itemsPerPage > discordUtilities.EmbedLimits.MaximumFields) throw new Error(`Pages cannot contain more than ${discordUtilities.EmbedLimits.MaximumFields} fields.`);
    this.totalPages = Math.ceil(this.items.length / this.itemsPerPage);
    this.generatePages();
    return this;
  }
...
var canReadMessagesPermissions = new discord_js.PermissionsBitField([discord_js.PermissionFlagsBits.ViewChannel]);
...
var canSendMessagesPermissions = new discord_js.PermissionsBitField([canReadMessagesPermissions, discord_js.PermissionFlagsBits.SendMessages]);
...
var canSendEmbedsPermissions = new discord_js.PermissionsBitField([canSendMessagesPermissions, discord_js.PermissionFlagsBits.EmbedLinks]);
...
var canSendAttachmentsPermissions = new discord_js.PermissionsBitField([canSendMessagesPermissions, discord_js.PermissionFlagsBits.AttachFiles]);
...
var canReactPermissions = new discord_js.PermissionsBitField([
...
var canRemoveAllReactionsPermissions = new discord_js.PermissionsBitField([
...
var canJoinVoiceChannelPermissions = new discord_js.PermissionsBitField([discord_js.PermissionFlagsBits.Connect]);
...
exports.actionIsLinkButton = actionIsLinkButton;
...
exports.isActionChannelMenu = isActionChannelMenu;
exports.isActionLink = isActionLink;
exports.isActionMentionableMenu = isActionMentionableMenu;
exports.isActionRoleMenu = isActionRoleMenu;
exports.isActionStringMenu = isActionStringMenu;
exports.isActionUserMenu = isActionUserMenu;
...
exports.isMessageChannelSelectInteractionData = isMessageChannelSelectInteractionData;
exports.isMessageInstance = isMessageInstance;
exports.isMessageMentionableSelectInteractionData = isMessageMentionableSelectInteractionData;
exports.isMessageRoleSelectInteractionData = isMessageRoleSelectInteractionData;
exports.isMessageStringSelectInteractionData = isMessageStringSelectInteractionData;
exports.isMessageUserSelectInteractionData = isMessageUserSelectInteractionData;
...
Object.keys(discordUtilities).forEach(function (k) {
  if (k !== 'default' && !Object.prototype.hasOwnProperty.call(exports, k)) Object.defineProperty(exports, k, {
    enumerable: true,
    get: function () { return discordUtilities[k]; }
  });
});
